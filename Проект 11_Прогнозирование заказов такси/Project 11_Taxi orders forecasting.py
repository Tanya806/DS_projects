...
# 
# Если строить прогноз на длительный период времени, то без учёта всех видов сезонности не обойтись.<br>
# Например, если в данных восходящий тренд, то возникает: <br>
...
#     
# Совет:<br>
# на горизонте прогнозирования несколько часов или дней предположить что у нас тренд.<br>
...
#     <b><u>КОММЕНТАРИЙ</u></b>
# 
# Не все факторы подходят логически:<br><br>
# 
...
# </font>
# <font color='green'><br> Хорошо. А по поводу учесть начало и конец месяца (или другие периоды месяца) - хорошая идея. Её можно реализовать так: создать новый столбец из 0 и 1. Где 1 промаркеровать те даты (события), которые хотим включить, как факторы. А 0 - остальные дни.<br>
# 2) и 3)  - ОК.
...
# <div style="background: #cceeaa; padding: 5px; border: 1px solid green; border-radius: 5px;">
# <font color='green'> 
#     <b><u>КОММЕНТАРИЙ</u></b><font color='green'> по фактору лаг и скользящему среднему:<br>
# Lag: его называют ещё параметром авторегрессии. 
# Допустим lag=1, он учитывает (для удобства будем считать, что продажи собраны по дням) как вчерашние продажи (вчерашние события) поалияли на сегоняшние. Lag=2: учитывает как позавчерашние продажи влияютна сегодняшние и т.д.<br>
#     
...
# Его легко увидеть на пределе: допустим длина ряда 100 дней, тогда rolling(100) - это просто одно число, среднее всего ряда. <br>
#     
...
# 1) Модели нужно подсказать причины, по которым происходит колебания продаж (временного ряда): понимать причины колебаний треда, понимать аномалии, учитывать акционные продажи, видеть действия конкурентов , закладывать их в модель и т.д. <br><br>
# 
# 2) Важны три группы факторов: <br>
...
# 
# 3)  важно иметь ввиду горизонт прогноза: на кокой период строить прогноз нужно  - на час вперёд, на неделю, на месяц.<br> Это накладывает ограничения на подбор моделей<br>
# Удобно составить матрицу  в разрезе: <br>
...
# </font>
# 
# Поэтой теме можно посмотреть практические материалы здесь:<br>
# https://ibf.org/knowledge - заморский институт бизнес-прогнозирования.<br>
# <br>
# 
# Есть классический труд (он НЕ ML, а исключительно на эксель). Но для понимания сути временных рядов - то, что надо:<br>УИЧЕРН "Бизнес-прогнозирование"<br>
# 
