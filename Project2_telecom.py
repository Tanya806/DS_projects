...
print('Тариф SMART')
print('-----------')
smart.groupby('month')['user_id'].count().plot(x='month',y=i,kind='bar') 
...
print('Тариф ULTRA')
print('-----------')
ultra.groupby('month')['user_id'].count().plot(x='month',y=i,kind='bar') 
plt.title('Распределение числа пользователей тарифом по месяцам')
plt.show()
...
#Шаг 5. 
#1) Проверим гипотезу "Средняя выручка пользователей тарифов «Ультра» и «Смарт» различаются"
